...
import { appendToFile, parseDateOrNull } from "./utils/helpers";
...
async function main() {
  ...
  while (stop === false) {
    ...
    const res = await fetch(next ?? url, options);
    ...
    for (let i = 0; i < json.data.length; i++) {
      ...
      const data = {
        ...
        date_of_birth: new Date(
          member.attributes.properties.bant_date_of_birth
        ),
        ...
        student_sit_in:
          member.attributes.properties.bant_student_sit_in === "1",
        ...
        cnhc_exemption:
          member.attributes.properties.bant_cnhc_exemption === "1",
        ...
        cnhc_exemption_rsn:
          member.attributes.properties.bant_cnhc_exemption_rsn,
      ...
      storeData(data, (i+1) * j + i, current);
      if (i % 7 === 0) await new Promise((res) => setTimeout(res, 1000));
    }
    j++;
  }
}

export async function storeData(membershipData: any, i: number, url: string) {
  try {
    ...
    appendToFile(
      `failed writing to db: ${membershipData.member_id}  url:: ${url} \n`,
      "error.txt"
    );
  } finally {
